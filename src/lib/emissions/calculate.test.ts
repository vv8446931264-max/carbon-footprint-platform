...
describe("calculateEmissionsKgCo2e", () => {
  ...
  it("computes waste emissions as factor * weight, with recycling reducing impact", () => {
    const landfill: Activity = {
      category: "waste",
      wasteType: "landfill",
      weightKg: 10,
    };
    const recycled: Activity = {
      category: "waste",
      wasteType: "recycled",
      weightKg: 10,
    };
    expect(calculateEmissionsKgCo2e(landfill)).toBeGreaterThan(
      calculateEmissionsKgCo2e(recycled),
    );
  });
...
describe("suggestSwap", () => {
  it("suggests train over petrol car and quantifies the saving", () => {
    ...
    expect(suggestion!.alternativeKgCo2e).toBeLessThan(
      suggestion!.originalKgCo2e,
    );
  });

  it("returns null when no lower-carbon alternative is defined", () => {
    expect(
      suggestSwap({ category: "transport", mode: "bike", distanceKm: 10 }),
    ).toBeNull();
    expect(
      suggestSwap({ category: "food", food: "beef", quantityKg: 1 }),
    ).toBeNull();
  });
});
