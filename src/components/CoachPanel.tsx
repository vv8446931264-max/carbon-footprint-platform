 ...
 */
export function CoachPanel({ totalKgCo2e, periodDays, topCategories }: CoachPanelProps) {
    ...
    try {
      const response = await fetch("/api/coach", {
        ...
        body: JSON.stringify({
          totalKgCo2e,
          periodDays,
          topCategories: topCategories.map(({ category, kgCo2e }) => ({ category, kgCo2e })),
        }),
      });
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 id="coach-heading" className="text-sm font-medium text-zinc-500 dark:text-zinc-400">
        ...
        >
          {status === "loading" ? "Thinking…" : report ? "Regenerate report" : "Get my report"}
        </button>
      </div>
      ...
      <div role="status" aria-live="polite" className="mt-4">
        ...
        {!report && status !== "error" && totalKgCo2e === 0 && (
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Log a few activities to unlock your report.</p>
        )}

        {report && (
          <div className="flex flex-col gap-3">
            <p className="text-sm text-zinc-800 dark:text-zinc-100">{report.summary}</p>
            <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">{report.encouragement}</p>
